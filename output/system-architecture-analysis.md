...
1. **Analyze the source code above** and extract ONLY the essential information for a simple system architecture diagram
...
### For System Architecture Diagram - Extract Essentials:

**Extract:**
- Main components/services/modules (with brief purpose)
- System layers (frontend, backend, database, cache, etc.)
- Communication channels between components (APIs, messages, events)
- External dependencies (databases, third-party services, APIs)
- Key data flows and interactions

**Output:**
Concise prompt with: components list, layer structure, connections as "Component A → Component B: protocol/method". Include only main architectural elements.
---

## Summary

**Your Task:** Extract ONLY essential system architecture information that actually exists in the source code above. Generate a simple, token-efficient prompt for another LLM to create a basic system architecture diagram. 
